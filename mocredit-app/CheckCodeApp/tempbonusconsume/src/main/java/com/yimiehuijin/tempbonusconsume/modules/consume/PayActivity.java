...
public class PayActivity extends IBaseActivity implements OnClickListener {
	...
	protected void init() {
		...
		if (bin != null && !bin.isEmpty()) {
			...
			if(ar != null && ar.activityList != null) {
				for (ActivityInfo ai : ar.activityList) {
					if (ai.cardBin != null &&(ai.cardBin.contains(bin) || "".equals(ai.cardBin))) {
						ailist.add(ai);
					}
				}
			}
	...
	private void checkCodeResult(BonusConsumeResponse response) {
		...
		i.putExtra("result", "ok".equals(response.state)?true:false);
	...
	protected void findView() {
		...
		back.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				bonus.activitId = adapter.getSelectedItem().activitId;
