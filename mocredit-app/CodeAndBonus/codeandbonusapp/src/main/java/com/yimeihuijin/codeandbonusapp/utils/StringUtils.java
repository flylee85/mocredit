...
import com.yimeihuijin.commonlibrary.utils.RSAHelper;
...
public class StringUtils {
	...
	public static String getH() {
		String tmp = DeviceModel.getInstance().getDevice().getDevMD5() + App.rkey;
		if (tmp.length() != 48) {
			return null;
		}
	...
	public static String getH(String rkey) {
		String tmp = DeviceModel.getInstance().getDevice().getDevMD5() + rkey;
		if (tmp.length() != 48) {
			return null;
		}
		return RSAHelper.encryptByPublicKey(tmp, Constants.TEST_PUBLIC_KEY);
	...
	public static synchronized String getOrderId() {
		...
		tmpId = key;
		return DeviceModel.getInstance().getDevice().en + key;
	}

	public static String getPrintInfo(VO.AO info, String orderid) {
		...
		info.receiptPrint = info.receiptPrint.replace(Constants.TEMPLATE_DEVICE_CODE,
				info.deviceCode==null?"":DeviceModel.getInstance().getDevice().en);
	...
	public static int getInt(String str,int def){
		try{
			return Integer.parseInt(str,def);
		}catch (Exception e){
			return 0;
		}
	}
