...
//				}
	...
	public boolean isVerifyMAC(MessageObject request) throws FormatException {
		try {
			...
			if (msgType == null) {
				return false;
			} else if (msgType.startsWith("03") || msgType.startsWith("11")|| msgType.startsWith("06") || msgType.startsWith("08") || msgType.startsWith("11")) {
				return false;
			} else if (msgType.startsWith("01") || msgType.startsWith("02") || msgType.startsWith("04")|| msgType.startsWith("12")
					|| msgType.startsWith("13")|| msgType.startsWith("14")) {
		...
		try {
			...
			if ("string".equals(elementType)) {
				if (Constant.COMPRESS_ON == compress) {
					...
					byte[] value;
					if (id == 60 || id == 48 || id == 36 || id == 32 || id == 62|| id == 62) {
