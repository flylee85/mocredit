...
public class Decoder {
	...
	private static DecodeDetail decodeDetail = new DecodeDetail();
	@Autowired
	...
	public MessageObject decode(Object message,BinarySource source,XMLResult result, String flag) throws FormatException {
		...
		try {
			String byteStr = message.toString().substring(6);
			...
			for(int i=0;i<elementRequest.getContentSize();i++) {
				...
				if("06".equals(elementRequest.getContent(i).toString().substring(16,18)))
					posRequest.setField06(elementRequest.getContent(i).getValue());
				if("09".equals(elementRequest.getContent(i).toString().substring(16,18)))
					posRequest.setField09(elementRequest.getContent(i).getValue());
				...
				if("43".equals(elementRequest.getContent(i).toString().substring(16,18)))
					posRequest.setField43(elementRequest.getContent(i).getValue());
...
//			if(flag!=null && flag.equals("0"))
	...
	private MessageObject getBusinessCode(MessageObject request) {
		...
		if("0320".equals(messageType) || "0500".equals(messageType) || "0620".equals(messageType)
		|| "0800".equals(messageType) || "0820".equals(messageType)
		|| "1100".equals(messageType)| "1260".equals(messageType) || "0210".equals(messageType)){
			/* 获取60.3域域值*/
			String field60_3 = request.getField11_System_Trace_Audit_Number().substring(0,2);
			tranCode.append(field60_3);
		}else if( "0810".equals(messageType)|| "1110".equals(messageType)|| "1270".equals(messageType)|| "1320".equals(messageType)
				|| "1280".equals(messageType)|| "0830".equals(messageType)){
			String field60_3 = request.getField03_Processing_Code().substring(0,2);
			tranCode.append(field60_3);
//			tranCode.append("001");
		} else {
			/* 向tranCode中拼接3域的前两位*/
	...
	private MessageObject handleMAC(MessageObject request, byte[] bodySource) throws FormatException {
		/* 管理类交易不校验MAC，不转加密PIN*/
		String messageType = request.getMesstype();
		if ("0100".equals(messageType) || "0320".equals(messageType)
			|| "0500".equals(messageType) || "0620".equals(messageType)
			|| "0800".equals(messageType) || "0820".equals(messageType)
			|| "1100".equals(messageType) || "1110".equals(messageType)
			|| "0810".equals(messageType)|| "0830".equals(messageType)
			|| "0210".equals(messageType)) {
			return request;
		}
		...
		String resultMac = MacECBUtils.clacMac(HexBinary.decode("4142434445464748"), dataSource);
...
//		String mac = MacECBUtils.clacMac(HexBinary.decode("3131313131313131"), HexBinary.decode("02007020048020C09811160000010000849150000000000000004100000076021000370000010000849150D0000001455001000000003130323034343835303030303030303030303131313438313536BB10D776B118AF2B2000000000000000000822000001"));
		String mac = MacECBUtils.clacMac(HexBinary.decode("3131313131313131"), HexBinary.decode("128000A0048020C000115A535948313120202020000193022000324392258781701034D1409101140214563939393939393939303030303030303030303030303030000803000001"));
		System.out.println(mac);
		
		System.out.println(HexBinary.decode("3131313131313131"));
	}
}
