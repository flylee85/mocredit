...
import cn.m.common.hibernate3.MyBeanUtils;
import cn.m.common.hibernate3.Updater;
import cn.m.common.hibernate3.restrictions.Between;
import cn.m.common.hibernate3.restrictions.Eq;
import cn.m.common.hibernate3.restrictions.Ge;
import cn.m.common.hibernate3.restrictions.In;
import cn.m.common.hibernate3.restrictions.Le;
import cn.m.common.hibernate3.restrictions.Nullable;
import cn.m.common.hibernate3.restrictions.Or;
import cn.m.common.hibernate3.restrictions.OrderBy;
import cn.m.common.page.Pagination;
import cn.m.mt.dao.BaseDao;
...
public abstract class BaseDaoImpl<T extends Serializable> implements BaseDao<T> {
	...
	@SuppressWarnings("unchecked")
	public List findObject(Finder finder) {
	...
	public List<T> findByEgList(T eg, Condition[] conds,
			String... exclude) {
		Criteria crit = getCritByEg(eg, false, conds, exclude);
		return crit.list();
	}
	public List<T> findByEgList(T eg,
			String... exclude) {
		Criteria crit = getCritByEg(eg, false, null, exclude);
		return crit.list();
	}
	@SuppressWarnings("unchecked")
	public List<T> findByEgList(T eg, boolean anyWhere, Condition[] conds,
	...
	public Pagination<T> findByEg(T eg, boolean anyWhere, Condition[] conds,
	...
	public Pagination<T> findByEg(T eg, Condition[] conds,
			int page, int pageSize, String... exclude) {
		Order[] orderArr = null;
		Condition[] condArr = null;
		if (conds != null && conds.length > 0) {
		...
		Criteria crit = getCritByEg(eg, false, condArr, exclude);
		return findByCriteria(crit, page, pageSize, null, orderArr);
	}
	public Pagination<T> findByEg(T eg, 
			int page, int pageSize, String... exclude) {
		Order[] orderArr = null;
		Condition[] condArr = null;
		Criteria crit = getCritByEg(eg, false, condArr, exclude);
	...
	public int countQueryResult(Finder finder) {
		if(finder.getOrigHql().indexOf("group by")>0){
			Query query = getSession().createQuery(finder.getOrigHql());
			finder.setParamsToQuery(query);
	...
	protected int countQueryResult(Criteria c) {
		...
		try {
			orderEntries = (List<CriteriaImpl.OrderEntry>) MyBeanUtils.getFieldValue(impl,
					"orderEntries");
			MyBeanUtils.setFieldValue(impl, "orderEntries", new ArrayList<CriteriaImpl.OrderEntry>());
		} catch (Exception e) {
	...
	protected Criteria getCritByEg(T bean, boolean anyWhere, Condition[] conds,
		...
		if (conds != null) {
			for (Condition o : conds) {
				if (o instanceof OrderBy) {
					...
					Between bt=(Between)o;
					crit.add(Restrictions.between(bt.getField(),bt.getStart(),bt.getEnd()));
				} else if(o instanceof Ge){
					Ge ge=(Ge)o;
					crit.add(Restrictions.ge(ge.getField(), ge.getPram()));
				} else if(o instanceof Le){
					Le le=(Le)o;
					crit.add(Restrictions.le(le.getField(), le.getPram()));
				} else if(o instanceof Or){
					Or or=(Or)o;
					crit.add(Restrictions.or(or.getCondl(),or.getCondr()));
				}else if(o instanceof Eq){
					Eq eq = (Eq)o;
					crit.add(Restrictions.eq(eq.getField(), eq.getPram()));
			}else if(o instanceof In){
				In in = (In)o;
				if(in.getInlist().size() != 0){
					crit.add(Restrictions.in(in.getField(), in.getInlist()));
				}
			}
			}
		}
		...
		for (String field : fieldNames) {
			...
			if (id != null) {
				...
				String[] subfieldNames = subCm.getPropertyNames();
				for(String subfield:subfieldNames){
					Object o2 = subCm.getPropertyValue(o, subfield, POJO);
					if (o2 == null) {
						continue;
					}
					ClassMetadata sub2Cm = getCmd(o2.getClass());
					if (sub2Cm == null) {
						continue;
					}
					Serializable id2 = sub2Cm.getIdentifier(o2, POJO);
					if (id2 != null) {
						Serializable idName = sub2Cm.getIdentifierPropertyName();
						crit.add(Restrictions.eq(subfield + "." + idName, id2));
					}else{
						crit.createCriteria(field+"."+subfield).add(Example.create(o2));
					}
				}
	...
	public Pagination<Object[]> findPage(Finder finder, int pageNo, int pageSize) {
		int totalCount = countQueryResult(finder);
