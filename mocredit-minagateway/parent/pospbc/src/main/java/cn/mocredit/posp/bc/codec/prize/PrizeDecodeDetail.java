 ...
 * @author superdo
 */
public class PrizeDecodeDetail {
	private static Logger logger = Logger.getLogger(PrizeDecodeDetail.class);
	...
	public Element deCode(byte[] message, int length, String charset, String rootEleName) 
		...
		Element root = null;
		try {
			String pathRoot = "conf/";
			UP_LoadSchema1.loadSchema(pathRoot);
			Document schemaDoc = UP_LoadSchema1.getSchema();
			if(null==schemaDoc)
	...
	protected void decodeMessage(ByteBuffer recvBuf, Element schemaRoot,
		...
		try {
			...
			String messTypeName = UP_LoadSchema1.getElementNameById(0);
			...
			Element bitmap = new Element(UP_LoadSchema1.getElementNameById(1));
