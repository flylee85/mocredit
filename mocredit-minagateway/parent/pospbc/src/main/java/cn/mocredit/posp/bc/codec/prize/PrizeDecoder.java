...
public class PrizeDecoder {
	public PrizeDecoder() {
		
	}
	public PrizeDecoder(SD_TERM_KEY_Dao sD_TERM_KEY_dao) {
		SD_TERM_KEY_dao = sD_TERM_KEY_dao;
	}
	SD_TERM_KEY_Dao SD_TERM_KEY_dao;
	private static Logger logger = Logger.getLogger(PrizeDecoder.class);
	private static PrizeDecodeDetail decodeDetail = new PrizeDecodeDetail();
...
//			if(flag!=null && flag.equals("0"))
	...
	private MessageObject getBusinessCode(MessageObject request) {
		...
		if("0900".equals(messageType) || "0500".equals(messageType) || "0620".equals(messageType)
		|| "0800".equals(messageType) || "0820".equals(messageType)){
			/* 获取60.3域域值*/
			String field03_1 = request.getField03_Processing_Code().substring(0,2);
			tranCode.append(messageType);
			tranCode.append(field03_1);
		} else if ( "0000".equals(messageType)) {
			return request;
		}else {
			/* 向tranCode中拼接3域的前两位*/
...
//			tranCode.append(request.getField60_Reserved_Private().substring(0, 2));
	...
	private MessageObject handleMAC(MessageObject request, byte[] bodySource) throws FormatException {
		/* 管理类交易不校验MAC，不转加密PIN*/
		String messageType = request.getMesstype();
		if ("0000".equals(messageType) || "0900".equals(messageType) || "0910".equals(messageType) || "0710".equals(messageType)
			|| "0500".equals(messageType) || "0620".equals(messageType)
			|| "0800".equals(messageType) || "0820".equals(messageType)
			|| "0200".equals(messageType)) {
			return request;
		}
...
//		String resultMac = MacECBUtils.clacMac(HexBinary.decode("3131313131313131"), dataSource);
	...
	public static void main(String[] args) {
		String mac = MacECBUtils.clacMac(HexBinary.decode("3131313131313131"), HexBinary.decode("08100038000002C0001300000317422012303936383736353433323131323334353637383930313233343500110000000100100003303120"));
