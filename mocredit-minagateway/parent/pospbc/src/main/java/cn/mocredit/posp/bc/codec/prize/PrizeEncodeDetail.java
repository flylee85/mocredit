...
import cn.mocredit.posp.bc.binding.Constant;
import cn.mocredit.posp.bc.binding.UP_LoadSchema1;
import cn.mocredit.posp.bc.binding.ValidateFFormat8583;
import cn.mocredit.posp.bc.binding.ValidateField;
import cn.mocredit.posp.bc.binding.ValidateVFormat8583;
import cn.mocredit.posp.bc.util.BCD;
import cn.mocredit.posp.bc.util.BitMap;
import cn.mocredit.posp.bc.util.CodecUtil;
import cn.mocredit.posp.bc.util.HexBinary;
import cn.mocredit.posp.bc.util.RidFillBlank;
 ...
 *
 */
public class PrizeEncodeDetail {
	private static Logger logger = Logger.getLogger(PrizeEncodeDetail.class);
	...
	private void encodeMessage(MessageObject request,
		...
		try {
			...
			String messageTypeName = UP_LoadSchema1.getElementNameById(0);
			...
			for (int i = 1; i < bs.length(); i++) {
				...
				Element element = UP_LoadSchema1.getElementById(i);
				/*get attribute name*/
				String elementName = element.getAttributeValue("name");
				if(flag.equals("0") && elementName.equals(UP_LoadSchema1.getElementNameById(64))) {
...
//				}
				...
				Element format = UP_LoadSchema1.getFormatElementByName(elementName);
				...
				if (Constant.KIND_F.equals(kind)) {
					...
					ValidateVFormat8583 vVFormat = new ValidateVFormat8583(elementName, elementType, format, UP_LoadSchema1.getNamespace());
					/*修改 传入域号 为了60域特殊处理*/
	...
	public boolean isVerifyMAC(MessageObject request) throws FormatException {
		try {
			...
			if (msgType == null) {
				return false;
			} else if (msgType.startsWith("09") || msgType.startsWith("07") || msgType.startsWith("08") || msgType.startsWith("05")) {
				return false;
			} else if (msgType.startsWith("01") || msgType.startsWith("02") || msgType.startsWith("04")) {
		...
		try {
			...
			if ("string".equals(elementType)) {
				if (Constant.COMPRESS_ON == compress) {
					...
					byte[] value;
					if (id == 60 || id == 48 || id == 36 || id == 32 || id == 62) {
