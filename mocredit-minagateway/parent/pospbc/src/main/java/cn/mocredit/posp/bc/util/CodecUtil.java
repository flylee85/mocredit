	 ...
	 public static Iterator<?> getElements(XmlSchemaType schemaType)
		    throws FormatException
		  {
		    ...
		    if (!(complexType.getParticle() instanceof XmlSchemaSequence)) {
		      throw new FormatException("can not find the child element sequence!");
		    }
		    ...
		    throws FormatException
		  {
		    ...
		    if (null == element.getAnnotation()) {
		      throw new FormatException("element:[" + elementName + "],can not find the child element [annotation]");
		    }
		    ...
		    for (Iterator it = annColl.getIterator(); it.hasNext(); ) {
		      ...
		      if ((o instanceof XmlSchemaAppInfo)) {
		        ...
		        if (null == format) {
		          throw new FormatException("element:[" + elementName + "],can not find the child element [format]");
		        }
		      }
		    }
		    if (null == format)
		      throw new FormatException("element:[" + elementName + "],can not find the element [appinfo]");
		    return format;
		  }
		  public static org.w3c.dom.Element getChildEleOfAppinfo(NodeList list, String childElemName) {
		  ...
		  private static boolean simpleElementHasFormat(XmlSchemaElement element) throws FormatException
		  {
		    boolean result = false;
		    if (null == element.getAnnotation()) {
		      throw new FormatException("element:[" + element.getName() + "],can not find the child element [annotation]");
		    }
		  ...
		  private static boolean complexElementHasFormat(XmlSchemaElement element, XmlSchema schema) throws FormatException
		  {
		    boolean result = false;
		    ...
		    if (it.hasNext()) {
		      ...
		      XmlSchemaType elementType = childElement.getRefName() != null ? schema.getElementByName(childElement.getRefName()).getSchemaType() : childElement.getSchemaType();

		      if (childElement.getRefName() != null)
		        childElement = schema.getElementByName(childElement.getRefName());
		  ...
		  public static boolean hasFormat(XmlSchemaElement element, XmlSchema schema) throws FormatException
		  {
		    boolean result = false;
		    XmlSchemaType xsType = element.getRefName() != null ? schema.getElementByName(element.getRefName()).getSchemaType() : element.getSchemaType();
		  ...
		  public static org.jdom.Element getComplexType(Document schemaDoc) throws FormatException {
		    org.jdom.Element root = schemaDoc.getRootElement();
		    Namespace ns = root.getNamespace();
		    org.jdom.Element request;
		    try {
		      XPath xpath = XPath.newInstance("xsd:element[@name='request']");
