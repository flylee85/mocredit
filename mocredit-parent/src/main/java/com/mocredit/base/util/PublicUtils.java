...
public class PublicUtils {
  ...
  public static int getDayNumForDate(Date startDate, Date endDate) {
    int day = 0;
    day = Integer.parseInt(String.valueOf((endDate.getTime() - startDate.getTime())
        / (24 * 60 * 60 * 1000)));
    if (day < 0) {
      day = 0;
    }
  ...
  public static int getDayNumForDate(Date startDate, String endDate) {
    ...
    try {
      day = Integer
          .parseInt(String.valueOf((formart.parse(endDate).getTime() - startDate.getTime())
              / (24 * 60 * 60 * 1000)));
      if (day < 0) {
        day = 0;
      }
  ...
  public static String isNullFormartForXSS(Object o) {
    if (null != o) {
//    return unescapeFromHTML(String.valueOf(o));
  ...
  public static String unescapeFromHTML(String target) {
    ...
    target = target.replaceAll("&#39;", "'");
    target = target.replaceAll("&nbsp;", " ");
    target = target.replaceAll("&mdash;", "——");
    target = target.replaceAll("&ndash;", "-");
    target = target.replaceAll("&ldquo;", "“");
    target = target.replaceAll("&rdquo;", "”");
    target = target.replaceAll("&sbquo;", "‚");
    target = target.replaceAll("&lsquo;", "‘");
    target = target.replaceAll("&rsquo;", "’");
    target = target.replaceAll("&tilde;", "˜");
    target = target.replaceAll("&amp;", "&");
    return target;
  }
  ...
  public static Double credit(String num, String count, int size) {
    return Math.round((Integer.parseInt(String.valueOf(num)) / (Integer.parseInt(String
        .valueOf(count)) * size)) * 100) / 100.00;
  }
  ...
  public static void main(String[] args) {
    System.out.println(unescapeFromHTML("你好\n&lt;&gt;\n&#39;&#39;\n,\n"));
  }
}
