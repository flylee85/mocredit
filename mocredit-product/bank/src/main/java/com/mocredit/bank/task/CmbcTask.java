...
import com.mocredit.bank.util.DesEncrypt;
...
public class CmbcTask {
	...
	public void qianDao() {
		...
		for (TiShopMerchant merchant : merchants) {
			MessageObject req = new MessageObject();
			req.setMesstype("0800");
			/** 获取终端流水号+1,初次签到默认为 000001 */
			req.setField11_System_Trace_Audit_Number(String.format("%06d", Integer.parseInt(merchant.getPosNo()) + 1));
			/** 设置终端编号 */
			req.setField41_Card_Acceptor_Terminal_ID(merchant.getTerminalId());
			/** 设置商户编号 */
			req.setField42_Card_Acceptor_ID(merchant.getMerchantId());
			/**
			 * 设置60.1交易类型码、60.2批次号、60.3网络管理信息吗
			 * 001-单倍长密钥算法终端用001,003-双倍长密钥算法终端用003
			 */
			req.setField60_Reserved_Private("00" + merchant.getBatchNo() + "003");
			/** 设置操作员，默认为01加一个空格 */
			...
			try {
				...
				// pos.setRETCODE(f39 == null || f39.length() == 0 ? "EMPTY" :
				// f39);
				if ("00".equals(f39)) {
					/** 签到成功，设置虚拟终端信息表 */
					merchant.setStatus(MerchantStatus.SIGNIN.getStatus());// 设置签到状态1-正常
																			// 3-签到
																			// 4-签退
																			// 2-异常
					/** 跟新终端流水号、批次号、更新时间 */
					merchant.setPosNo(String.format("%06d", Integer.parseInt(merchant.getPosNo()) + 1));
					merchant.setBatchNo(res.getField60_Reserved_Private().substring(2, 8));// 设置批次号
					...
					/* 设置lmktpk */
					merchant.setLmkTpk(field62.substring(0, 32));
					/* 设置lmktak */
					merchant.setLmkTak(field62.substring(40, 72));
					// 民生提供的主密钥
					// String lmkTmk = "FD4FF48ADA3E6E102013A24A04D36298";
					// costa咖啡测试2014-03-14测试
					// String lmkTmk = "A4F8D99EF116529E8F67EFD3047A3438";
					/* 设置tmktak */
					merchant.setTmkTak(DesEncrypt.decryptDES(merchant.getLmkTmk(), field62.substring(40, 72)));
					/* 设置tmktpk */
					// sd_TERM_KEY.setTMK_TPK(DesEncrypt.decryptDES(lmkTmk,
					// field62.substring(0,32)));
				} else {
					merchant.setStatus(MerchantStatus.INVALID.getStatus());
				}
				merchant.setUpdateTime(DateTimeUtils.getDate("yyyyMMddHHmmss"));// 设置更新时间
	...
	// * @throws Exception
	// */
	// private void gxhdlb(String term_ID, String merch_ID, String batch_NO)
	// throws Exception {
	// // 获得活动列表
	...
	// }
	// } while (true);
	// String hql = "from SD_POINTS_INFO where TREM_ID='" + term_ID + "' and
	// MERCH_ID='" + merch_ID + "'";
	// List<SD_POINTS_INFO> infoList = SD_POINTS_INFO_dao.find(new Finder(hql));
	// HashMap<String, SD_POINTS_INFO> map = new HashMap<String,
	// SD_POINTS_INFO>();
	...
	// String hdxq =
	// "3"+posAction.manageTran(req).getField62_Reserved_Private();
	// info.setPOINTS(new String(decode(hdxq.substring(30, 50)),"GBK").trim());
	// info.setPRODUCT_NAME(new String(decode(hdxq.substring(50, 170)),
	// "GBK").trim());
	// }
	// }
	//
	...
	// return (bk == null || bk.size() != 1) ? "" :
	// bk.get(0).getId().toString();
	// }
	//
	// /**
	// * 将字符串从某一位开始截取某长度的若干段
	...
	// /**获取终端流水号+1,初次签到默认为 000001*/
	// req.setField11_System_Trace_Audit_Number(String.format("%06d",
	// Integer.parseInt(sdPosTerm.getPOS_NO())+1));
	// /**设置终端编号*/
	...
	// sdPosTerm.setBATCH_NO(res.getField60_Reserved_Private().substring(2,
	// 8));//设置批次号
	...
	// /**获取终端流水号+1*/
	// req.setField11_System_Trace_Audit_Number(String.format("%06d",
	// Integer.parseInt(pos.getPOS_NO())+1));
	// /**设置终端编号*/
	...
	// }
	// String format = String.format("%012d",
	// Integer.parseInt(sum.replaceAll("\\.", "")));
	...
	//// String s =
	// "";
	...
	//// task.qianDao();
	//// String a =
	// "30303030303030303030303032343335303020202020202020ccd7b2cd4220202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020202020";
	//// try {
