	 ...
	 public static String ridRightBlank(byte[] buf, int blank, String type, String charset)
			    ...
			    if (type.equals("string")) {
			      for (int i = buf.length - 1; i >= 0; i--) {
			        if ((0 == (blank ^ buf[i])) || (0 == (binaryZero ^ buf[i]))) continue;
			        try {
			  ...
			  public static String fillBlank(String elementValue, int length, char blank, String align, String encodeCharset, String elementName)
			    throws FormatException
			  {
			    ...
			    if (len > length) {
			      throw new FormatException("element:[" + elementName + "],length of elementValue:[" + elementValue + "] is more longer then xsd defind len:[" + length + "]");
			    }
			    if (len == length) return elementValue;
