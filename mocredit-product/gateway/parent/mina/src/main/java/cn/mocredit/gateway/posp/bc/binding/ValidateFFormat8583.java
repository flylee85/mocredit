...
public class ValidateFFormat8583 {
	  ...
	  public ValidateFFormat8583(org.jdom.Element element, org.jdom.Element format) throws FormatException
	  {
	    String type = element.getAttributeValue("type");
	    this.id = Integer.parseInt(format.getAttributeValue("id"));
	    if ("string".equals(type)) {
	      String lenStr = format.getAttributeValue("length");
	      this.len = ValidateField.validateLength(element.getAttributeValue("name"), lenStr);

	      this.align = format.getAttributeValue("align", "L");
	      this.align = ValidateField.validateAlign(element.getAttributeValue("name"), this.align);

	      String blankStr = format.getAttributeValue("blank");
	      this.blank = ValidateField.validateBlank(element.getAttributeValue("name"), this.blank, blankStr);

	      String compressStr = format.getAttributeValue("compress");
	      this.compress = ValidateField.validateCompress(element.getAttributeValue("name"), compressStr);
	    } else if ("int".equals(type))
	    {
	      this.endian = format.getAttributeValue("endian", "big");
	      this.endian = ValidateField.validateEndian(element.getAttributeValue("name"), this.endian);
	    }
	    ...
	    {
	      throw new FormatException("element:[" + element.getAttributeValue("name") + "],attribute type is:[" + type + "] error!");
	    }
	  }
	    ...
	    throws FormatException
	  {
	    ...
	    if ("string".equals(type)) {
	      ...
	      String blankStr = format.getAttributeValue("blank");
	      this.blank = ValidateField.validateBlank(elementName, this.blank, blankStr);

	      String compressStr = format.getAttributeValue("compress");
	      this.compress = ValidateField.validateCompress(elementName, compressStr);
	    } else if ("int".equals(type))
	    {
	      this.endian = format.getAttributeValue("endian", "big");
	      this.endian = ValidateField.validateEndian(elementName, this.endian);
	    }
	    else if ("hexBinary".equals(type))
	    {
	      String lenStr = format.getAttributeValue("length");
