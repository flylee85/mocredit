 ...
 * 
 * @Description     活动同步_导入_再次同步
 * 
 * */

public class Activity_Synchronization_Import_100_ImportAgain_Test {
	/**
	 * 活动同步参数列表
	 * 
     *  activityId     活动ID
	...
	@Test
	public void ActivitySynchronization_Import_ImportAgain(String baseUrl)throws Exception {
		...
		String endTime_before_starttime = PropertyUtil.getProInfo("parmeter", "endTime_before_starttime");
		String selectDate_notexist = PropertyUtil.getProInfo("parmeter", "selectDate_notexist");
		String integral = PropertyUtil.getProInfo("parmeter", "integral");
		String maxType_notexist = PropertyUtil.getProInfo("parmeter", "maxType_notexist");
...
//////////*********************************活动新增导入**********************************///////		
		...
		// 定义endTime字段
		Buffer.append("\"endTime\"").append(":").append("\"").append(endTime_before_starttime).append("\"");
		// 定义selectDate字段
		Buffer.append("\"selectDate\"").append(":").append("\"").append(selectDate_notexist).append("\"");
		// 定义storeNintegralame字段
		Buffer.append("\"integral\"").append(":").append("\"").append(integral).append("\"");
		// 定义maxType字段
		Buffer.append("\"maxType\"").append(":").append("\"").append(maxType_notexist).append("\"");
		// 定义maxNumber字段
		...
		// 再次活动同步
		String resp2 = HttpRequestUtil.doPostJson(baseUrl + "activityImport", jsonStr);
		//转换为字符串
		Response response2 = JSON.parseObject(resp2, Response.class);
		//返回异常时的结果判定
		Assert.assertEquals(response2.getSuccess(), false);
		Assert.assertEquals(response2.getErrorCode(), "503");
		Assert.assertEquals(response2.getErrorMsg(), "已存在该活动");
