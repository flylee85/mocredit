 ...
 * 
 * @Description     积分消费_银行为未签约银行
 * 
 * */

public class Integral_consume_010_BankUnsign_Test {
	...
	@Test
	public void IntegralConsume_BankUnsign(String baseUrl)throws Exception {
		Random r = new Random();
		int i = 1 + r.nextInt(9000);
		String bank = PropertyUtil.getProInfo("parmeter", "bank_null");
		...
		Assert.assertEquals(response.getErrorCode(), "008");
		Assert.assertEquals(response.getErrorMsg(), "未签约银行");
	}
}
