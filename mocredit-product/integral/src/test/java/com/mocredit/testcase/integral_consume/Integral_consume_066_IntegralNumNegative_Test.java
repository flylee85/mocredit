 ...
 * 
 * @Description     积分消费_积分为负值
 * 
 * */

public class Integral_consume_066_IntegralNumNegative_Test {
	...
	@Test
	public void IntegralConsume_IntegralNumNegative(String baseUrl)throws Exception {
		Random r = new Random();
		...
		// 第二次传入相同参数
		String resp1 = HttpRequestUtil.doPostJson(baseUrl + "payment", jsonStr);
		//转换为字符串
		Response response1 = JSON.parseObject(resp1, Response.class);
		//返回异常时的结果判定
		Assert.assertEquals(response1.getSuccess(), false);
		Assert.assertEquals(response1.getErrorCode(), "505");
		Assert.assertEquals(response1.getErrorMsg(), "参数错误");
