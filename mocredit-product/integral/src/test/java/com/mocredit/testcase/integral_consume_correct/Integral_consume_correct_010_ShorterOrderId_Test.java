 ...
 * 
 * @Description     积分消费冲正_原订单号参数过短
 * 
 * */

public class Integral_consume_correct_010_ShorterOrderId_Test {
	...
	@Parameters({ "baseUrl"})
	@Test
	public void IntegralConsumeCorrect_ShorterOrderId(String baseUrl)throws Exception {
		Random r = new Random();
...
////////定义积分消费冲正缓冲区
		...
		// 定义integral字段
		Buffer2.append("\"orderId\"").append(":").append("\"").append(orderId_shorter).append("\"");
		// 定义缓冲区结束
		Buffer2.append("}");
		// 字符串转换成jsonStr
