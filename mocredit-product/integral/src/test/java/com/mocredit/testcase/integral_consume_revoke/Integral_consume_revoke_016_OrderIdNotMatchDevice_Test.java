 ...
 * 
 * @Description     积分消费撤销_订单号与设备号不匹配
 * 
 * */

public class Integral_consume_revoke_016_OrderIdNotMatchDevice_Test {
	...
	@Test
	public void IntegralConsumeRevok_OrderIdNotMatchDevice(String baseUrl)throws Exception {
		Random r = new Random();
		...
		String orderId_nomatch = "hegytest" + i;
...
////////定义积分消费撤销缓冲区
		...
		// 定义integral字段
		Buffer2.append("\"orderId\"").append(":").append("\"").append(orderId_nomatch).append("\"");
		// 定义缓冲区结束
		Buffer2.append("}");
		// 字符串转换成jsonStr
...
///////进行积分消费		
		// 传入消费撤销参数
		String resp3 = HttpRequestUtil.doPostJson(baseUrl + "paymentRevoke", jsonStr2);
		//转换为字符串
		Response response3 = JSON.parseObject(resp3, Response.class);
		//返回异常时的结果判定
		Assert.assertEquals(response3.getSuccess(), false);
		Assert.assertEquals(response3.getErrorCode(), "006");
		Assert.assertEquals(response3.getErrorMsg(), "交易记录不存在");
