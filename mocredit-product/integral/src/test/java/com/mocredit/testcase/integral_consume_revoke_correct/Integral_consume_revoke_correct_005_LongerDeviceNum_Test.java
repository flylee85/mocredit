 ...
 * 
 * @Description     积分消费撤销冲正_终端号参数长度过长
 * 
 * */

public class Integral_consume_revoke_correct_005_LongerDeviceNum_Test {
	/**
	 * 积分消费参数列表
	 * 
	...
	@Parameters({ "baseUrl"})
	@Test
	public void IntegralConsumeRevokeCorrect_LongerDeviceNum(String baseUrl)throws Exception {
		Random r = new Random();
		...
		String device_longer = PropertyUtil.getProInfo("parmeter", "device_longer");
...
////////定义积分消费撤销冲正缓冲区
		...
		Buffer3.append("{");
		// 定义cardNum字段
		Buffer3.append("\"device\"").append(":").append("\"").append(device_longer).append("\"");
		// 定义integral字段
