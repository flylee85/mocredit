 ...
 * 
 * @Description     积分消费撤销冲正_订单号长度过短
 * 
 * */

public class Integral_consume_revoke_correct_010_ShorterOrderId_Test {
	/**
	 * 积分消费参数列表
	 * 
	...
	@Parameters({ "baseUrl"})
	@Test
	public void IntegralConsumeRevokCorrect_ShorterOrderId(String baseUrl)throws Exception {
		Random r = new Random();
...
////////定义积分消费撤销冲正缓冲区
		...
		// 定义integral字段
		Buffer3.append("\"orderId\"").append(":").append("\"").append(orderId_shorter).append("\"");
		// 定义缓冲区结束
		Buffer3.append("}");
		// 字符串转换成jsonStr
