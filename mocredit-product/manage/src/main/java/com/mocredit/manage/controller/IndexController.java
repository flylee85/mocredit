...
public class IndexController extends ShiroBaseController {
	...
	public ModelAndView loginAction(HttpServletRequest request, String username, String password) {
		...
		getMenus(PropertiesUtil.getValue("client.app.key"), PropertiesUtil.getValue("client.app.loginUrl"),
				JSON.toJSONString(param), request);
	...
	//// List<Resource> menus = (List<Resource>)
	// SecurityUtils.getSubject().getSession().getAttribute("menus");
	// mav.addObject("menus",
	// getMenus(PropertiesUtil.getValue("client.app.key"),
	// PropertiesUtil.getValue("client.app.loginUrl")));
	// return mav;
	// }
	/*
	 * @RequestMapping("/logout") public ModelAndView logout() {
	 * //定义页面对象，用来跳转指定页面，下面一句代码的意思是：跳转到Web容器中的index.jsp页面
	 * SecurityUtils.getSubject().logout(); ModelAndView mav = new
	 * ModelAndView("redirect:index"); return mav; }
	 */
