...
public class ActivityCodeServiceImpl implements ActivityCodeService {
	 ...
	 * @return
	 */
	public AjaxResponseData useActivityCodeWithTran(String device, String store_id, String store_code,
			String request_serial_number, String code) {
		...
		if (activityCode.isEffective()) { // 属于有效的
			// List<TVerifiedCode> verCodes =
			// vcm.findVerifiedCodesByActiveCodeId(activityCode.getId());
			// int nextTimes = null == verCodes ? 1 : verCodes.size() + 1;
			// //存放当前应该为第几次请求。
			//
			...
			// if( activityCode.getMaxNum() >=
			// Constant.ACTIVITY_MORE_TIMES_NUMBER ){
			// isQuotas=false;
			// }
			// //标示是否可以进行验码核销的数据操作。
			...
			//
			// Map<String , TVerifiedCode> excludeMap = new HashMap<String,
			// TVerifiedCode>();
			// for (TVerifiedCode obj : verCodes) {
			// //计算金额
			// if (
			// obj.getVerifyType().equals(VerifyCodeStatus.CORRECT.getValue() )
			...
			// 计算使用的次数：" + currentUsedTimes);
			// }
			// logger.debug("###### 最终计算：有效使用次数=" + currentUsedTimes + " ;
			// 有效的使用金额=" + currentUsedAmount);
			//// for (TVerifiedCode obj : verCodes) {
			//// //计算金额
			//// if
			// (obj.getVerifyType().equals(VerifyCodeStatus.CORRECT.getValue())
			...
			// obj.getVerifyType().equals(VerifyCodeStatus.REVOKE.getValue())) {
			//// currentUsedAmount =
			// currentUsedAmount.subtract(obj.getAmount());
			//// } else if
			// (obj.getVerifyType().equals(VerifyCodeStatus.VERIFYCODE.getValue()))
			// {
			//// currentUsedAmount = currentUsedAmount.add(obj.getAmount());
			//// }
			//// logger.debug("#### 计算使用金额：" + currentUsedAmount);
			...
			//
			// remainAmount =
			// activityCode.getAmount().subtract(currentUsedAmount).subtract(amount.multiply(new
			// BigDecimal(useCount))); // 此次使用次数乘以金额
			// if (activityCode.getMaxNum() >=
			// Constant.ACTIVITY_MORE_TIMES_NUMBER
			// || (remainTimes >= useCount && remainAmount.compareTo(new
			// BigDecimal(0.00)) > -1)) {
			// canVerifyCode = true;
			// }
			// }
			//
			//
			// logger.debug("######## 判断是否可以继续核销：" + canVerifyCode + " \r 剩余次数="
			// + remainTimes + " ;剩余金额=" + remainAmount);
			...
			// dataMap.put(TemplateUtil.TemplateField.AMOUNT ,String.valueOf(
			...
			Map<String, String> httpPostMap = new HashMap<>();
			...
			httpPostMap.put("timestamp", DateUtil.dateToStr(tvc.getVerifyTime(), "yyyyMMddHHmmss"));
			httpPostMap.put("code", code);
			httpPostMap.put("activityId", activityCode.getActivityId());
			HttpUtil.doRestfulByHttpConnection(sysOrderUrl, JSON.toJSONString(httpPostMap));
			/* 同步到订单接口 结束 */
			// } else {
			// ard.setSuccess(false);
			// if (remainTimes < useCount) {
	...
	public AjaxResponseData correctActivityCodeWithTran(String request_serial_number, String code, String device,
			VerifyCodeStatus verifyCodeStatus) {
		AjaxResponseData ard = new AjaxResponseData();

		// 判断是否属于冲正与撤销
		if (verifyCodeStatus.getValue() != VerifyCodeStatus.REVOKE.getValue()
				&& verifyCodeStatus.getValue() != VerifyCodeStatus.CORRECT.getValue()) {
			ard.setSuccess(false);
		...
		// 获取对应的
		Date date = new Date();
		List<TVerifiedCode> findCodes = this.vcm.findVerifiedCodesByDeviceAndRequestSerialNumber(device,
				request_serial_number, date);
		...
		/*
		 * if( null!=findCodes && findCodes.size()>0){ TActivityCode
		 * activityCode=findCodes.get(0);
		 * 
		 * // 验码核销存在 按照次数 或者 金额 核销。使用数量与金额不会同时使用
		 * //遍历取得的列表。如果按照次数，则是多条（金额为0）。如果是金额（如果按照金额，应该是一条。）则设置金额即可。
		 * List<TVerifiedCode>
		 * verifiedCodes=this.vcm.findVerifiedCodesByCodeAndRequestSerialNumber(
		 * code,request_serial_number); for( TVerifiedCode v:verifiedCodes ){
		 * TVerifiedCode newV=new TVerifiedCode();
		 * newV.setActivityId(v.getActivityId());
		 * newV.setActivityName(v.getActivityName());
		 * newV.setAmount(v.getAmount());
		 * newV.setCodeSerialNumber(v.getCodeSerialNumber());
		 * newV.setCode(v.getCode()); newV.setDevice(v.getDevice());
		 * newV.setShopId(v.getShopId()); newV.setShopName(v.getShopName());
		 * newV.setEndTime(activityCode.getEndTime()); //
		 * newV.setIssueEnterpriseName(v.getIssueEnterpriseName());
		 * newV.setIssueEnterpriseId(v.getIssueEnterpriseId());
		 * newV.setId(UUIDUtils.UUID32()); newV.setNumber(-1); //根据数据库设计，撤销和冲正填写
		 * 负数 newV.setRequestSerialNumber(v.getRequestSerialNumber());
		 * newV.setStartTime(activityCode.getStartTime());
		 * newV.setStoreId(v.getStoreId()); newV.setStoreName(v.getStoreName());
		 * newV.setVerifyType( verifyCodeStatus.getValue()); newV.setVerifyTime(
		 * new Date()); this.vcm.insertVerifiedCode(newV ); }
		 * ard.setSuccess(true); Map m=new HashMap();
		 * m.put("activityId",activityCode.getActivityId());
		 * m.put("activityName",activityCode.getActivityName());
		 * m.put("startTime",activityCode.getStartTime());
		 * m.put("endTime",activityCode.getEndTime() );
		 * m.put("issueEnterpriseId",activityCode.getIssueEnterpriseId());
		 * m.put("issueEnterpriseName",activityCode.getIssueEnterpriseName());
		 * ard.setData(m); }else{ ard.setSuccess(false);
	...
	public AjaxResponseData updateActActivityWithTran(ActActivityCodeVO actActivityCodeVO) {
		...
		// 4.解除黑名单的锁定
		int delete_blacklist_c = this.activityCodeBlackListsMapper
	...
	public AjaxResponseData chexiao(String orderId,String code) {
		List<TVerifiedCode> verifiedCodes = vcm.findVerifiedCodesByCodeAndRequestSerialNumber(code, orderId);
		if(null!=verifiedCodes&&!verifiedCodes.isEmpty()){
			TActivityCode activityCode = new TActivityCode();
			activityCode.setStatus(ActivityCodeStatus.NOT_USED.getValue());
			activityCode.setId(verifiedCodes.get(0).getCodeSerialNumber());
			acm.updateActivityCode(activityCode );
		}
		AjaxResponseData ard = new AjaxResponseData();
		return ard;
