...
public class AppDaoImpl implements AppDao {
    ...
    public App findOneByUserId(Long userId) {
        final String sql = "select b.* from sys_user_app_roles a LEFT JOIN sys_app b on a.app_id=b.id where a.user_id=?";
        List<App> appList = jdbcTemplate.query(sql, new BeanPropertyRowMapper(App.class), userId);
        if (appList.size() == 0) {
            return null;
        }
        return appList.get(0);
    ...
    public List<App> findAppByList(Integer page, Integer pageSize) {
        ...
        final String sql = "select id, name, app_key, app_secret, available from sys_app limit ?,?";
        return jdbcTemplate.query(sql, new BeanPropertyRowMapper(App.class), offset, pageSize);
    }
}
