...
//        final String sql = "select id, user_id, app_id, role_ids as roleIdsStr from sys_user_app_roles where app_id=? and user_id=?";
        final String sql = "select id, user_id, app_id, role_ids as roleIdsStr from sys_user_app_roles where user_id=?";
        List<Authorization> authorizationList = jdbcTemplate.query(sql, new BeanPropertyRowMapper<Authorization>(Authorization.class), userId);
        if (authorizationList.size() == 0) {
            return null;
        }
        return authorizationList.get(0);
    ...
    public void updateAuthorizationRoleByUserId(Long userId, Long roleId) {
        final String sql = "update sys_user_app_roles set role_ids=? where user_id=?";
